...
coreFreq = [1, 2, 3, 4]
price_limit = 12000
max_number_of_sequential_machine = 1 # можэет быть инициализирована как максимальное колво параллельных задач или как максимальон еоличестов последоватльных задачи всего
...
def greedy_algorithm_for_sequential_tasks(CPU, coreFreq, price_limit, max_number_of_sequential_machine):
    ...
    try:
        while True and number_of_machine <= max_number_of_sequential_machine:
            ...
            machine = ConfigurationOfMachines(1, 2, 16)
...
def greedy_algorithm_for_configuration_of_machine(CPU, coreFreq, price_limit):
    ...
    try:
        while True and number_of_machine <= 15:
            ...
            for i in selected_machines_sequential:
                machine = ConfigurationOfMachines(len(selected_machines_sequential), i[1], i[0])
                machines_for_sequential.add_machine(machine)
                machine_price = machine.price
                pricesOfUsingMachines[machine.id] = machine.price
    ...
    return machines_for_sequential, machines_for_parallel, cost_of_current_package, working_time_of_current_package, scheduling_table_of_current_package, class_machines_parallel
...
selected_machines_sequential, selected_machines_parallel, cost_of_current_package, working_time_of_current_package, scheduling_table_of_current_package, class_machines_parallel_greedy = greedy_algorithm_for_configuration_of_machine(CPU, coreFreq, price_limit)
print("в часах", working_time_of_current_package / 3600)
