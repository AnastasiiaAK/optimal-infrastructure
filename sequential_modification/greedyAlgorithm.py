...
def greedy_algorithm_for_sequential_tasks(CPU, coreFreq, price_limit, max_number_of_sequential_machine):
    ...
    sorted_list_of_switch = list(i["number_attached"] for i in dictSwitches.values())  # указана максимальная вместимость для каждого свитча
    print(sorted_list_of_switch)
    ...
    # для свитча
    '''
    iter_switch = iter(sorted_list_of_switch)
    potential_switch = next(iter_switch)
    selected_switch = 1
    global working_time_of_current_package
    working_time_of_current_package = math.inf
    ...
    try:
        while True:
            machines_for_parallel, machines_for_sequential = from_list_machine_configuration(selected_machines_sequential, [(2, 1)], 1)
    '''
...
def greedy_algorithm_for_configuration_of_machine(CPU, coreFreq, price_limit):
    ...
    try:
        while True and number_of_machine < 16:
            ...
            if price_of_all_working < price_limit: # добавим ограничение время на тарнсфер бменьше времени на выполнение
