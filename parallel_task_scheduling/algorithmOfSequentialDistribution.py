...
def scheduling_for_non_paralleling_task(id_of_task, current_time, machines, tasks_dataframe):
    ...
    # найти количество машин одновременно передающих
    if len(tasks_dataframe[(tasks_dataframe["possibilityOfParalleling"] > 0.3) & (tasks_dataframe["done"] == "Yes")]) <= len(machines.listOfMachines):
        number_of_machine = len(machines.listOfMachines)
    else:
        number_of_machine = 1



    transfer_time = machines.switch.calculate_transfer_time(number_of_machine, tasks_dataframe.loc[tasks_dataframe["indexOfTask"] == id_of_task])
    ...
    transfer_time1 = machines.switch.calculate_transfer_time_to(number_of_machine, tasks_dataframe.loc[tasks_dataframe["indexOfTask"] == id_of_task])
    ...
    # найдем количесвт машин одновременно используемых в момент времени
    ...
    if len(free_machines) == 0:
        ...
        for device in machines.listOfMachines:
            if time_of_nearest_free_machine > device.time_of_deliverance:
                time_of_nearest_free_machine = device.time_of_deliverance
    ...
    best_free_machine.make_machine_busy(current_time)
...
def scheduling_for_paralleling_task(id_of_task, current_time, machines, tasks_dataframe):
    ...
    tasks_dataframe.at[id_of_task, "executingTime"] = worst_time  # time with transfer to machine
