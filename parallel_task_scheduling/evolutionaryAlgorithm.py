...
from parallel_task_scheduling.greedyAlgorithm import greedy_algorithm_for_sequential_tasks, analytical_algorithm_for_sequential_tasks


max_number_of_machine = max_number_of_sequential_machine
CPU = [2, 4, 8, 16]
...
def calculate_time_and_price_of_current_set_of_machines(sample_of_machines):
    ...
    switch_sequential = ConfigurationOfSwitches(len(sample_sequential))
...
def calculate_time_and_price_of_current_set_of_sequential_machines(sample_of_machines):
    ...
    price_of_all_working = sum(scheduling_table["executingPrice"]) + sum(scheduling_table["transferPrice"])
    common_time = max(scheduling_table["endTime"])
    return common_time, price_of_all_working
...
def evaluate_sequential(individual):
    ...
    if price_of_all_working > common_price_for_all_sequential_tasks:
        return math.inf,
    elif sum(individual[0]) > max_number_of_sequential_machine or sum(individual[0]) < 1:
        return math.inf,
    else:
        return common_time,
...
if max_number_of_machine > 1:
    sequential_set, best_fit = evolution_algorithm_for_parallel_tasks(max_number_of_machine, len(sorted_list_of_configuration_machine_sequential), evaluate_sequential , 25, 250) # max_number_of_machine, max_possible_configuration, evaluate, number_of_generation, size_of_population, maximum_values_of_machine
...
best_machine_evolution, _ = evolution_algorithm_for_parallel_tasks(16, len(sorted_list_of_configuration_machine), evaluate, 50, 500) # max_number_of_machine, max_possible_configuration, evaluate, number_of_generation, size_of_population
