...
proportion_of_parallel_tasks = round(descriptionOffTask
                                     [descriptionOffTask["possibilityOfParalleling"] > 0.3]["complexityOfTask"].sum() / 16
                                     / descriptionOffTask["complexityOfTask"].sum(), 5)

proportion_of_sequential_tasks = round((descriptionOffTask[descriptionOffTask["possibilityOfParalleling"] > 0.3]["complexityOfTask"].sum() * 15/16
                                       + descriptionOffTask[descriptionOffTask["possibilityOfParalleling"] <= 0.3]["complexityOfTask"].sum())
                                       / descriptionOffTask["complexityOfTask"].sum(), 5)
...
def proportion_of_price_for_different_type_of_tasks(price_limit, proportion_of_parallel_tasks, proportion_of_sequential_tasks):
    common_price_for_all_sequentuial_tasks = price_limit * proportion_of_sequential_tasks
    common_price_for_all_parallel_tasks = price_limit * proportion_of_parallel_tasks
